...
insert into _etablissement(uai, nom_etablissement, secteur, code_postal, lattitude, longitude, code_insee_de_la_commune, nom_de_la_commune, code_academie, code_nature)
  ...
  from _temp_fr_en_etablissements_ep as temp
  where uai is not null and nom_etablissement is not null and statut_public_prive is not null and code_postal is not null and latitude is not null and longitude is not null and code_commune is not null and nom_commune is not null and code_academie is not null and code_nature is not null
    and exists (select 1 from _commune WHERE _commune.code_insee_de_la_commune = temp.code_commune and _commune.nom_de_la_commune = upper(temp.nom_commune));
...
insert into _caracteristiques_tout_etablissement(uai, annee_scolaire, effectifs)
  ...
  from _temp_fr_en_ips_colleges_ap2022 as temp
  where effectifs is not null
    and exists (select 1 from _etablissement WHERE _etablissement.uai = temp.uai);

------------------------
-- _quartier prioritaire
...
drop constraint if exists est_a_proximite_de_fk_qp cascade,
add constraint est_a_proximite_de_fk_qp foreign key(code_quartier_prioritaire, nom_quartier_prioritaire) references _quartier_prioritaire(code_quartier_prioritaire, nom_quartier_prioritaire);
...
insert into _est_a_proximite_de(code_quartier_prioritaire, uai, nom_quartier_prioritaire)
  ...
  from _temp_fr_en_etablissements_ep as temp
  where qp_a_proximite is not null and nom_du_qp is not null
    and exists (select 1 from _etablissement WHERE _etablissement.uai = temp.uai);
...
insert into _caracteristiques_college(uai, annee_scolaire, nbre_eleves_hors_segpa_hors_ulis, nbre_eleves_segpa, nbre_eleves_ulis, ips, ecart_type_de_l_ips, ep)
  select distinct uai, ic.rentree_scolaire, (nombre_eleves_total - (nombre_eleves_segpa + nombre_eleves_ulis)), nombre_eleves_segpa, nombre_eleves_ulis, ips, ecart_type_de_l_ips, false
  from _temp_fr_en_ips_colleges_ap2022 ic left outer join _temp_fr_en_college_effectif_niveau_sexe_lv ce
  on ic.uai = ce.numero_college and ic.nom_de_l_etablissment = ce.denomination_principale
  where nombre_eleves_total is not null and nombre_eleves_segpa is not null and nombre_eleves_ulis is not null and ips is not null and ecart_type_de_l_ips is not null and rep is not null
    and exists (select 1 from _etablissement WHERE _etablissement.uai = ic.uai) on conflict do nothing;

----------------
--- _caracteristiques_selon_classe


drop table _temp_fr_en_college_effectif_niveau_sexe_lv;
drop table _temp_fr_en_etablissements_ep;
drop table _temp_fr_en_indicateurs_valeur_ajoutee_colleges;
drop table _temp_fr_en_ips_colleges_ap2022;
drop table _temp_communes;
